...
class MultiKeyClient:
    ...
    def chat(self, messages: List[Dict[str, str]], max_tokens=5000, temperature=0.3) -> str:
        max_attempts = 8
        for attempt in range(1, max_attempts + 1):
            ...
            if self.lightning_semaphore.acquire(blocking=False):
                ...
                # Don't print warning here to avoid spam, just instantly overflow.
...
def process_company(md_path: Path, sector: str, company: str, year: str, out_dir: Path, client: MultiKeyClient, difficulties: List[str], min_bundle_size: int, max_workers: int):
    ...
    for difficulty in difficulties:
        ...
        qa_file = diff_dir / f"test_{difficulty}_qa.jsonl"
        ...
        def worker(bundle):
            ...
            if difficulty == "table_reasoning" and not bundle.get("has_table", False):
                return
            ...
            try:
                ...
                for json_attempt in range(1, max_json_retries + 1):
                    ...
                    if valid_pairs is not None:
                        logger.info(f"✅ Successfully generated QA for bundle {bundle['bundle_id']}")
                        break
                    else:
                ...
                logger.error(f"Failed on bundle {bundle['bundle_id']}: {e}")
    ...
    for difficulty in difficulties:
        qa_file = out_dir / difficulty / f"test_{difficulty}_qa.jsonl"
    ...
    local_count_file = out_dir / "question_count.log"
    with local_count_file.open("w", encoding="utf-8") as f:
        f.write(f"Total Questions for {year}: {total_company_questions}\n")
        f.write("\nBy Difficulty:\n")
    ...
    local_hal_file = out_dir / "hallucination_count.log"
    with local_hal_file.open("w", encoding="utf-8") as f:
        f.write(f"Total Hallucinations for {year}: {total_company_hallucinations}\n")
...
def main():
    ...
    parser.add_argument("--difficulty", default="all", choices=["easy", "medium", "all"])
    ...
    logger.info(f"Loaded {len(api_keys)} Lightning API keys.")
    ...
    # Initialize the client first so we can read how many Nvidia keys you passed in
    ...
    # Automatically scale NVIDIA workers based on the number of keys (e.g. 8 workers per key)
    ...
    # Set up server_requests.log in the output root so it persists and resumes correctly
    server_log_path = output_root / "server_requests.log"
    ...
    difficulties = ["easy", "medium"] if args.difficulty == "all" else [args.difficulty]
    ...
    pbar = tqdm(all_reports, desc="Annual Reports", unit="report")
    for md_path, sector, company in pbar:
        ...
        logger.info(f"Processing {sector} / {company} / {year}")
        process_company(
            md_path=md_path,
            sector=sector,
            company=company,
        ...
        for qa_file in output_root.rglob("test_*_qa.jsonl"):
            with qa_file.open("r", encoding="utf-8") as f:
                for line in f:
                    global_total += 1
                    try:
                        ...
                        pass
        with (output_root / "global_question_count.log").open("w", encoding="utf-8") as f:
            f.write(f"Global Total Questions Generated: {global_total}\n")
            f.write("\nBy Question Type:\n")
        ...
        with (output_root / "global_hallucination_count.log").open("w", encoding="utf-8") as f:
            f.write(f"Global Total Hallucinations: {global_hal_total}\n")
                
    logger.info("Easy/Medium QA Generation Complete.")
